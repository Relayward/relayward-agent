...
The optional `--server-cert-sha256` value adds a leaf-certificate pin while retaining normal TLS verification. Plain HTTP is supported when `--allow-insecure` is explicitly set, but registration credentials, commands, and telemetry are then transmitted without encryption. Use HTTPS whenever possible. The center-generated installation command adds this flag automatically when its Public URL uses HTTP.
